...
country_codes = ['BGR', 'EST', 'GRC', 'HRV', 'KAZ', 'ESP', 'LVA', 'ITA', 'DNK', 'DEU', 'UNK', 'SWI', 'AUT', 'EUR', 'TUR', 'HUN', 'MDA', 'CZE', 'FIN', 'NLD', 'ALB', 'BEL', 'SVK', 'SRB', 'ARM', 'UKR', 'SCO', 'NOR', 'ROU', 'AZE', 'POL', 'FRA', 'SWE', 'PRT']
...
def parse_arguments():
    ...
    parser.add_argument('country_code', type=str, help='Country code (e.g., ENG, GER) or ALL for all countries')
...
def load_league_configs(country_code):
    ...
    """
    configs = []
    configs_dir = '/root/moneyball/Configs'
...
def process_country(country_code, matches_collection, leagues_seasons_df):
    """
    Process ELO updates for a single country code.
    """
    ...
    if not configs:
        logging.error(f"No league configurations found for country code '{country_code}'. Skipping.")
        return
    logging.info(f"Loaded configurations for {len(configs)} leagues for country code {country_code}.")
    ...
    for config in configs:
        ...
        config_country_code = config.get('country', 'UNK')
        ...
        for season, comp_id in competition_ids_dict.items():
            competition_info[comp_id] = {
                ...
                'country_code': config_country_code,
    ...
    # === Map Competition IDs to Seasons using leagues_seasons_df ===
    ...
    # === Load or Initialize ELO Ratings (country-specific file) ===
    ELO_RATINGS_FILE = f"/root/moneyball/tmp/elo_ratings_{country_code}.joblib"
    INITIAL_ELO = 1500
    ...
    END_OF_SEASON_ELOS_DIR = "/root/moneyball/elo/data/elo-data/end-of-season-data"
    ...
    logging.info(f"ELO update for country code {country_code} completed successfully.")

def main():
    # === Initialize Logging Early ===
    LOG_FILE_PATH = "/root/moneyball/logs/elo_update.log"
    setup_logging(LOG_FILE_PATH)
    logging.info("Logging initialized.")

    try:
        provided_country_code = parse_arguments()
        logging.info(f"Country code argument provided: {provided_country_code}")

        # === Connect to MongoDB (shared connection for all countries) ===
        ...
        # === Load Leagues Seasons CSV once ===
        LEAGUES_SEASONS_CSV = "/root/moneyball/tmp/all_leagues.csv"
        ...
        # === Process Countries ===
        if provided_country_code.upper() == "ALL":
            for code in country_codes:
                logging.info(f"Starting processing for country code: {code}")
                process_country(code, matches_collection, leagues_seasons_df)
        else:
            process_country(provided_country_code, matches_collection, leagues_seasons_df)
